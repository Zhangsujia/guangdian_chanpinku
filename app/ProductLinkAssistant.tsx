...
type Member = { email: string; displayName: string; role: "admin" | "member"; active: boolean; mustChangePassword?: boolean; createdAt: string };
...
type Draft = {
  ...
  links: Array<{ platform: string; url: string; mechanism?: string; commission?: number }>;
  awaiting: "name" | "price" | "link" | "mechanism" | "commission";
};

type QuickLinkRow = { id: string; platform: string; url: string; mechanism: string; commission: string };
type QuickPackageRow = { id: string; name: string; price: string; description: string };
type QuickEntry = { name: string; manufacturer: string; price: string; status: ProductStatus; packages: QuickPackageRow[]; links: QuickLinkRow[] };
...
const DEFAULT_TEAM: TeamBrand = { name: "产品链接管家", subtitle: "团队产品资料安全同步", themeColor: "#187657", avatarUrl: "", updatedAt: "" };
...
function TeamMark({ brand, className = "brand-mark" }: { brand: TeamBrand; className?: string }) {
  return <div className={className} style={{ backgroundColor: brand.themeColor }}>{brand.avatarUrl ? <img src={brand.avatarUrl} alt="团队头像" /> : <Icon name="bot" />}</div>;
}
...
export default function ProductLinkAssistant() {
  ...
  const [quickEntry, setQuickEntry] = useState<QuickEntry>({ name: "", manufacturer: "", price: "", status: "正常推广", packages: [], links: [] });
  const [sharedMechanism, setSharedMechanism] = useState("");
  ...
  function reply(text: string, products?: Product[]) {
    setMessages((current) => [...current, { id: uid("msg"), role: "assistant", text, products }]);
  }
  ...
  function parseIntoQuickEntry(text: string) {
    ...
    const links = parsed.links.length
      ? parsed.links.map((entry) => ({ id: uid("quick"), platform: entry.platform, url: entry.value, mechanism: parsed.mechanism, commission: parsed.commission === null ? "" : String(parsed.commission) }))
      : [{ id: uid("quick"), platform: "抖音", url: "", mechanism: parsed.mechanism, commission: parsed.commission === null ? "" : String(parsed.commission) }];
    ...
    setQuickEntry({ name: parsed.name, manufacturer: parsed.manufacturer, price: parsed.price === null ? "" : String(parsed.price), status: parsed.productStatus, packages, links });
    setActiveQuickLinkId(links[0]?.id ?? null);
    setSharedMechanism(parsed.mechanism);
    setSharedCommission(parsed.commission === null ? "" : String(parsed.commission));
  }
  ...
  function openProductEditor(product: Product) {
    ...
    const mechanisms = [...new Set(currentProduct.links.map((link) => link.mechanism.trim()).filter(Boolean))];
    const commissions = [...new Set(currentProduct.links.map((link) => link.commission))];
    setSharedMechanism(mechanisms.length === 1 ? mechanisms[0] : "");
    setSharedCommission(commissions.length === 1 ? String(commissions[0]) : "");
    setQuickImageFile(null);
  ...
  function addQuickPlatform() {
    ...
    setQuickEntry((current) => ({
      ...
      links: [...current.links, {
        ...
        url: "",
        mechanism: sharedMechanism.trim(),
        commission: validQuickCommission(sharedCommission) ? sharedCommission : "",
      }],
    }));
  ...
  function makeBatchEntry(input: ReturnType<typeof parseQuickProductInput>): BatchEntry {
    const links = input.links.length
      ? input.links.map((entry) => ({ id: uid("batch-link"), platform: entry.platform, url: entry.value, mechanism: input.mechanism, commission: input.commission === null ? "" : String(input.commission) }))
      : [{ id: uid("batch-link"), platform: "抖音", url: "", mechanism: input.mechanism, commission: input.commission === null ? "" : String(input.commission) }];
    ...
    return { id: uid("batch"), name: input.name, manufacturer: input.manufacturer, price: input.price === null ? "" : String(input.price), status: input.productStatus, packages, links, imageFile: null, imagePreview: "" };
  }
  ...
  function getBatchEntryIssues(entry: BatchEntry, entries = batchEntries) {
    ...
    if (entry.links.some((link) => !link.url.trim())) issues.push("存在空白链接");
    if (entry.links.some((link) => !link.mechanism.trim())) issues.push("存在未填写的机制");
    if (entry.links.some((link) => !validQuickCommission(link.commission))) issues.push("存在未填写或格式错误的佣金");
    ...
    const urls = entry.links.map((link) => link.url.trim()).filter(Boolean);
    const existingUrls = new Set(database.products.flatMap((product) => product.links.map((link) => link.url.trim())).filter(Boolean));
    if (urls.some((url) => existingUrls.has(url))) issues.push("产品库中已有相同链接");
    const batchUrlCount = new Map<string, number>();
    entries.forEach((item) => item.links.forEach((link) => {
      const url = link.url.trim();
      if (url) batchUrlCount.set(url, (batchUrlCount.get(url) ?? 0) + 1);
    }));
  ...
  async function chooseBatchTable(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      rows.slice(1).forEach((row) => {
        ...
        } else if (!entry.packages.length && columns.packages >= 0) {
        ...
        entry.links.push({
          id: uid("batch-link"),
          platform: columns.platform >= 0 ? row[columns.platform]?.trim() || inferPlatform(url) : inferPlatform(url),
          url,
  ...
  function downloadBatchTemplate() {
    const rows = [
      ["产品名称", "厂家", "单品价格", "产品状态", "套餐（选填）", "平台", "链接", "产品机制", "佣金"],
      ["示例胶原蛋白", "示例厂家", "39.9", "正常推广", "2盒69.9元；3盒99元", "抖音", "请替换为实际链接", "拍一发一", "35%"],
      ["示例胶原蛋白", "示例厂家", "39.9", "正常推广", "2盒69.9元；3盒99元", "视频号", "请替换为实际链接", "拍一发一", "35%"],
    ];
    const csv = "\uFEFF" + rows.map((row) => row.map(escapeCsv).join(",")).join("\r\n");
    downloadBlob(new Blob([csv], { type: "text/csv;charset=utf-8" }), "产品批量导入模板.csv");
  }

  function updateBatchEntry(id: string, patch: Partial<Pick<BatchEntry, "name" | "manufacturer" | "price" | "status">>) {
  ...
  function addBatchPlatform(entryId: string) {
    setBatchEntries((current) => current.map((entry) => {
      ...
      return { ...entry, links: [...entry.links, { id: uid("batch-link"), platform, url: "", mechanism: entry.links[0]?.mechanism ?? "", commission: entry.links[0]?.commission ?? "" }] };
    }));
  }
    ...
    try {
      for (let index = 0; index < batchEntries.length; index += 1) {
        ...
        const product: Product = {
          ...
          createdAt: timestamp,
          updatedAt: timestamp,
          links: entry.links.map((link) => ({ id: uid("link"), platform: link.platform, url: link.url.trim(), mechanism: link.mechanism.trim(), commission: Number(link.commission), status: "有效", updatedAt: timestamp })),
        };
  ...
  async function saveQuickEntry(event: FormEvent) {
    ...
    const incomplete = !quickEntry.name.trim() || !validQuickPrice(quickEntry.price)
      ...
      || !quickEntry.links.length || quickEntry.links.some((link) => !link.url.trim() || !link.mechanism.trim() || !validQuickCommission(link.commission));
    ...
    try {
      const saved = await saveCompletedDraft({
        ...
        awaiting: "mechanism",
        links: quickEntry.links.map((link) => ({ platform: link.platform, url: link.url.trim(), mechanism: link.mechanism.trim(), commission: Number(link.commission) })),
  ...
  async function saveCompletedDraft(completed: Draft, imageFile?: File | null, targetProductId?: string | null, removeImage = false) {
    ...
    if (!completed.name || completed.links.length === 0 || completed.links.some((link) => !link.mechanism?.trim())) {
      setDraft({ ...completed, awaiting: "mechanism" });
      reply("还缺少产品机制。请告诉我规格数量、买赠活动等内容；没有机制时链接不会保存。 ");
      return null;
    }
    if (completed.links.some((link) => link.commission === undefined)) {
    ...
    if (existing) {
      const links = completed.links.map((entry) => {
        ...
        const linkChanged = !previous || previous.url !== entry.url || previous.commission !== entry.commission;
        return {
    ...
    const lowCommission = saved.links.some((link) => link.commission < 30);
    const similarReminder = duplicates.similarProducts.length
      ...
      : "";
    reply(`${targetProductId ? "已更新" : "已保存"}“${saved.name}”，单品价 ${formatProductPrice(saved.price)}${saved.packages.length ? `，另有 ${saved.packages.length} 个套餐` : ""}。以后查询时，我会同步告诉你单品价、套餐、平台机制和佣金。${lowCommission ? "\n产品佣金低于30%，低于规定水平。" : ""}${similarReminder}`, [saved]);
    return saved;
  }
  ...
  function startAdd(text: string, imageFile: File | null = null, imagePreview = "") {
    ...
    const links = entries.map((entry) => ({ platform: entry.platform, url: entry.value, mechanism, commission }));
    ...
    void saveCompletedDraft({ name, manufacturer, price, status: parsed.productStatus, packages, links, awaiting: "mechanism" }, imageFile);
  }
  ...
  function continueDraft(text: string, current: Draft) {
    if (current.awaiting === "name") {
      ...
      const nextAwaiting = current.price === undefined ? "price" : !current.links.length ? "link" : current.links.some((link) => !link.mechanism) ? "mechanism" : "commission";
      const next = { ...current, name, awaiting: nextAwaiting as Draft["awaiting"] };
      if (current.price !== undefined && current.links.length && current.links.every((link) => link.mechanism && link.commission !== undefined)) {
      ...
      reply(current.price === undefined
        ...
        ? current.links.some((link) => !link.mechanism)
    ...
    if (current.awaiting === "price") {
      ...
      const next = { ...current, price, awaiting: (!current.links.length ? "link" : current.links.some((link) => !link.mechanism) ? "mechanism" : "commission") as Draft["awaiting"] };
      if (current.links.length && current.links.every((link) => link.mechanism && link.commission !== undefined)) void saveCompletedDraft(next);
    ...
    if (current.awaiting === "link") {
      ...
      const next: Draft = {
        ...current,
        links: entries.map((entry) => ({ platform: entry.platform, url: entry.value, mechanism, commission })),
    ...
    if (current.awaiting === "commission") {
      ...
      void saveCompletedDraft({ ...current, links: current.links.map((link) => ({ ...link, commission })) });
      return;
    }
    const mechanism = extractMechanism(text) ?? text.trim();
    if (mechanism.length < 2) return reply("机制内容太短了，请至少告诉我价格或活动信息。 ");
    const commission = extractCommission(text) ?? undefined;
    void saveCompletedDraft({ ...current, links: current.links.map((link) => ({ ...link, mechanism, commission: commission ?? link.commission })) });
  }
  ...
  function describeProductChanges(before: Product, after: Product) {
    ...
    platforms.forEach((platform) => {
      ...
      if (!previous && next) changes.push(`${platform}：恢复平台资料`);
        ...
        if (previous.mechanism !== next.mechanism) changes.push(`${platform}机制：${previous.mechanism} → ${next.mechanism}`);
        if (previous.commission !== next.commission) changes.push(`${platform}佣金：${previous.commission}% → ${next.commission}%`);
  ...
  function prepareConversationalEdit(text: string) {
    ...
    if (!isContextual && edit.fieldCount < 2) return false;
    ...
    const targets = product.links.filter((link) => !edit.platform || link.platform === edit.platform);
    if ((edit.mechanism !== undefined || edit.commission !== undefined) && !targets.length) {
      reply(`“${product.name}”还没有${edit.platform ?? "可修改的"}平台链接。`, [product]);
      return true;
    }
    const timestamp = nowIso();
    const links = product.links.map((link) => {
      if (edit.platform && link.platform !== edit.platform) return link;
      const commissionChanged = edit.commission !== undefined && edit.commission !== link.commission;
      return {
        ...link,
        mechanism: edit.mechanism ?? link.mechanism,
        commission: edit.commission ?? link.commission,
        updatedAt: commissionChanged ? timestamp : link.updatedAt,
      };
    });
  ...
  function queryByConditions(text: string) {
    ...
    const productMatches = database.products.flatMap((product) => {
      ...
      const needsLinkFilter = Boolean(platform || wantsStale || commissionAtLeast || commissionBelow || commissionAbove);
      const links = product.links.filter((link) => {
        if (platform && link.platform !== platform) return false;
        if (wantsStale && staleLinkDays(link) <= staleThreshold) return false;
        if (commissionAtLeast && link.commission < Number(commissionAtLeast[1])) return false;
        if (commissionBelow && link.commission >= Number(commissionBelow[1])) return false;
        if (commissionAbove && link.commission <= Number(commissionAbove[1])) return false;
        return true;
      });
  ...
  function runPreflightCheck(text: string) {
    ...
    links.forEach((link) => {
      if (link.status === "已失效") blockers.push(`${link.platform}链接已失效`);
      else if (link.status !== "有效") warnings.push(`${link.platform}链接状态为${link.status}`);
      const days = staleLinkDays(link);
      if (days > 7) warnings.push(`${link.platform}链接已${days}天未更新`);
      if (link.commission < 30) warnings.push(`${link.platform}佣金${link.commission}%，低于30%`);
    });
  ...
  function queryLinks(text: string) {
    const platform = PLATFORMS.find((item) => item !== "其他" && text.includes(item));
    const query = extractQueryName(text);
    ...
    const filtered = selected.map((product) => ({
      ...product,
      links: platform ? product.links.filter((link) => link.platform === platform) : product.links,
    ...
    reply(filtered.length > 1 ? "找到了几个相近的产品，请确认你要的是哪一个：" : `这是“${filtered[0].name}”的链接、单品价、套餐、产品机制和佣金：`, filtered);
  }
  ...
  function queryPrices(text: string) {
    ...
    const product = directlyNamed[0] ?? matches[0].product;
    setActiveProductId(product.id);
  ...
  function updateMechanism(text: string) {
    ...
    const links = product.links.map((link) => !platform || link.platform === platform ? { ...link, mechanism } : link);
    const updated = { ...product, links, updatedAt: nowIso() };
    setActiveProductId(product.id);
    commit({ ...database, products: database.products.map((item) => item.id === product.id ? updated : item) });
    reply(`已更新“${product.name}”${platform ? `的${platform}` : "全部平台"}机制。`, [updated]);
    return true;
  }
  ...
  function updateCommission(text: string) {
    ...
    const product = findProducts(database.products, query)[0];
    if (!product) { reply(`没有找到“${query}”。`); return true; }
    const links = product.links.map((link) => !platform || link.platform === platform ? { ...link, commission, updatedAt: nowIso() } : link);
    const updated = { ...product, links, updatedAt: nowIso() };
    setActiveProductId(product.id);
    commit({ ...database, products: database.products.map((item) => item.id === product.id ? updated : item) });
    reply(`已更新“${product.name}”${platform ? `的${platform}` : "全部平台"}佣金。${commission < 30 ? "\n产品佣金低于30%，低于规定水平。" : ""}`, [updated]);
    return true;
  }
  ...
  function updateProductStatus(text: string) {
    ...
    setActiveProductId(product.id);
    commit({ ...database, products: database.products.map((item) => item.id === product.id ? updated : item) });
    reply(`已把“${product.name}”设为${status}。${productStatusWarning(status)}`, [updated]);
    return true;
  }
  ...
  function updateLinkStatus(text: string) {
    ...
    setActiveProductId(product.id);
    commit({ ...database, products: database.products.map((item) => item.id === product.id ? updated : item) });
    reply(`已把“${product.name}”${platform ? `的${platform}` : "全部"}链接标记为${status}。`, [updated]);
    return true;
  }
  ...
  function requestProductEdit(text: string) {
    ...
    reply(
      products.length > 1
        ? `找到 ${products.length} 款相近产品，请在正确的卡片上点击“编辑资料”：`
        : `找到了“${products[0].name}”，点击卡片上的“编辑资料”即可一次修改全部信息：`,
      products,
    );
  ...
  function deleteByChat(text: string) {
    ...
    if (database.user?.role !== "admin") { reply("只有管理员可以删除产品。 "); return true; }
  ...
  function reportDuplicates() {
    ...
    const urlGroups = new Map<string, Array<{ product: Product; link: ProductLink }>>();
    database.products.forEach((product) => {
      ...
      product.links.forEach((link) => {
        const urlKey = link.url.trim();
        if (urlKey) urlGroups.set(urlKey, [...(urlGroups.get(urlKey) ?? []), { product, link }]);
      });
    });
  ...
  function handleText(raw: string) {
    ...
    if (/(哪些|查看|查询).*(失效|待复核)|失效链接|待复核链接/.test(text)) {
      const problem = database.products.map((product) => ({ ...product, links: product.links.filter((link) => link.status !== "有效") })).filter((product) => product.links.length);
    ...
    if (/(超过|超出)?\s*7\s*天.*(?:没|未).*更新|过期链接|陈旧链接/.test(text)) {
      const stale = database.products.map((product) => ({ ...product, links: product.links.filter((link) => staleLinkDays(link) > 7) })).filter((product) => product.links.length);
  ...
  async function checkLink(productId: string, linkId: string, url: string) {
    setChecking(linkId);
    try {
      ...
      const products = database.products.map((product) => product.id !== productId ? product : {
        ...
        links: product.links.map((link) => link.id !== linkId ? link : {
          ...
          checkNote: result.message,
          status: result.kind === "suspected" ? "疑似失效" as LinkStatus : link.status,
        }),
      });
  ...
  function exportCsv() {
    const header = ["产品名称", "厂家", "单品价格", "产品状态", "套餐（选填）", "平台", "链接", "产品机制", "佣金", "链接状态", "更新时间"];
    const lines = [header, ...database.products.flatMap((product) => product.links.map((link) => [
      product.name, product.manufacturer, String(product.price), product.status, formatPackages(product.packages), link.platform, link.url, link.mechanism, `${link.commission}%`, link.status, link.updatedAt,
    ...
    try {
      ...
      if (file.name.toLowerCase().endsWith(".json")) {
        ...
        const incomplete = parsed.products.some((product) => !Number.isFinite(product.price) || product.price <= 0
          ...
          || product.links.some((link) => !link.mechanism?.trim() || !Number.isFinite(link.commission)));
        if (incomplete) throw new Error("missing-required-fields");
        next = { ...database, products: parsed.products.map((product) => ({ ...product, status: PRODUCT_STATUSES.includes(product.status) ? product.status : "正常推广", price: Number(product.price ?? 0), packages: product.packages ?? [], imageUrl: product.imageUrl ?? "", revision: product.revision ?? 0, createdBy: product.createdBy ?? database.user?.email ?? "", updatedBy: product.updatedBy ?? database.user?.email ?? "" })) };
      } else {
        ...
        rows.slice(1).forEach((row) => {
          ...
          if (!product) {
            ...
            product = { id: uid("product"), name, manufacturer: row[col("厂家")]?.trim() || "未填写", price, status: PRODUCT_STATUSES.includes(importedStatus) ? importedStatus : "正常推广", packages, imageUrl: "", aliases: [], notes: "", revision: 0, createdBy: database.user?.email ?? "", updatedBy: database.user?.email ?? "", links: [], createdAt: timestamp, updatedAt: timestamp };
            products.push(product);
          }
          product.links.push({ id: uid("link"), platform: row[col("平台")]?.trim() || inferPlatform(url), url, mechanism, commission, status: (row[col("链接状态")] as LinkStatus) || (row[col("状态")] as LinkStatus) || "待复核", updatedAt: nowIso() });
        });
  ...
  function removeProduct(product: Product) {
    if (database.user?.role !== "admin") { setToast("只有管理员可以删除产品"); return; }
  ...
  function openBranding() {
    ...
    setBrandColor(brand.themeColor);
  ...
  const reviewCount = useMemo(() => database.products.reduce((sum, product) => sum + product.links.filter((link) => link.status !== "有效" || staleLinkDays(link) > 7).length, 0), [database]);
  const quickMissingCount = (quickEntry.name.trim() ? 0 : 1) + (validQuickPrice(quickEntry.price) ? 0 : 1)
    ...
    count + (link.url.trim() ? 0 : 1) + (link.mechanism.trim() ? 0 : 1) + (validQuickCommission(link.commission) ? 0 : 1)
  ), 0);
  ...
  const visibleProducts = useMemo(() => {
    const source = view === "review"
      ? database.products.map((product) => ({ ...product, links: product.links.filter((link) => link.status !== "有效" || staleLinkDays(link) > 7) })).filter((product) => product.links.length)
      : database.products;
    if (!search.trim()) return source;
    return source.filter((product) => fuzzyScore(product, search) >= 38 || product.manufacturer.includes(search));
  ...
  return (
    <main className="app-shell" style={{ "--green": activeBrand.themeColor, "--green-dark": activeBrand.themeColor, "--green-soft": `color-mix(in srgb, ${activeBrand.themeColor} 10%, white)` } as React.CSSProperties}>
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label="主要导航">
          ...
          {database.user?.role === "admin" && <button className={view === "trash" ? "active" : ""} onClick={() => setView("trash")}><Icon name="trash" /><span>回收站</span>{database.trash.length > 0 && <b>{database.trash.length}</b>}</button>}
      ...
      <section className="workspace">
        {view === "chat" ? (
          <>
            <header className="workspace-header">
              ...
              <div className="chat-header-actions">
                <button className="batch-entry-button" onClick={openBatchImport}>批量导入</button>
                <button className="quick-entry-button" onClick={() => openQuickEntry()}>＋ 快速录入</button>
            ...
            <div className="chat-feed">
              {messages.map((message) => (
                <div className={`message-row ${message.role}`} key={message.id}>
                  ...
                  <div className="message-stack">
                    ...
                    {message.products?.map((product) => <ProductResult key={`${message.id}-${product.id}`} product={product} onCopy={copy} onCheck={checkLink} onEdit={openProductEditor} checking={checking} />)}
                  </div>
                </div>
              ))}
              {database.products.length === 0 && messages.length === 1 && (
                <section className="starter-prompts">
                  <p>从这些话开始</p>
                  <div>
                    <button onClick={() => openQuickEntry()}><span>智能粘贴录入</span><small>整段资料一次识别</small></button>
            ...
            <div className="admin-body branding-body">
              <section className="admin-panel branding-panel">
                <div className="brand-preview-column">
                  ...
                  <p>保存后，侧边栏、聊天助手头像和主要按钮会同步使用新的团队外观。</p>
                </div>
          ...
          <>
            ...
            <div className="admin-body">
              ...
              <section className="admin-panel account-panel">
                <div><span className="section-kicker">当前账户</span><h2>{database.user?.displayName}</h2><p>{database.user?.email} · {database.user?.role === "admin" ? "管理员" : "普通成员"}</p></div>
          ...
          <>
            <header className="workspace-header library-header">
              <div><h1>团队成员</h1><p>成员可新增和修改产品，删除与恢复仅限管理员</p></div>
              <button className="signout-link" onClick={() => void logout()}>退出登录</button>
            </header>
            <div className="admin-body">
              ...
              <section className="admin-panel"><div className="panel-heading"><div><span className="section-kicker">当前团队</span><h2>{database.members.filter((member) => member.active).length} 位成员</h2></div><small>管理员可以删除和恢复产品</small></div><div className="member-list">{database.members.filter((member) => member.active).map((member) => <article key={member.email}><div className="member-avatar">{member.displayName.slice(0, 1).toUpperCase()}</div><div><strong>{member.displayName}</strong><span>{member.email}</span></div><em className={member.role}>{member.role === "admin" ? "管理员" : "普通成员"}</em>{member.role === "member" && <button onClick={() => void removeMember(member)}>停用</button>}</article>)}</div></section>
            </div>
          </>
        ) : view === "activity" ? (
          <>
          ...
          <>
            <header className="workspace-header library-header"><div><h1>回收站</h1><p>误删的产品可以由管理员恢复</p></div><span className="connection-pill">{database.trash.length} 款产品</span></header>
          ...
          <>
            <header className="workspace-header library-header">
              <div><h1>{view === "review" ? "待复核与超期链接" : "产品库"}</h1><p>{view === "review" ? "集中处理疑似失效、手动标记或超过7天未更新的链接" : "查看、检索和维护全部产品资料"}</p></div>
              <div className="header-buttons"><button onClick={exportCsv}><Icon name="download" />导出 Excel 清单</button><button onClick={openBatchImport}>批量导入</button><button className="primary" onClick={() => openQuickEntry()}>＋ 添加产品</button></div>
            </header>
            <div className="library-body">
              <div className="library-toolbar"><label><Icon name="search" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索产品名称或厂家" /></label><span>共 {visibleProducts.length} 款产品</span></div>
              {visibleProducts.length ? (
                <div className="product-list">
                  {visibleProducts.map((product) => <ProductRow key={product.id} product={product} onCopy={copy} onCheck={checkLink} onEdit={openProductEditor} onStatusChange={changeProductStatus} statusChanging={changingStatusId === product.id} onDelete={removeProduct} onImageChange={replaceProductImage} onImageRemove={removeProductImage} canDelete={database.user?.role === "admin"} onEditMechanism={(platform) => { setView("chat"); setInput(`把${product.name}的${platform}机制改成 `); }} checking={checking} />)}
                </div>
              ) : (
                <div className="empty-library"><div className="empty-icon"><Icon name={view === "review" ? "check" : "box"} /></div><h2>{view === "review" ? "没有需要复核的链接" : "产品库还是空的"}</h2><p>{view === "review" ? "目前所有链接状态正常，且最近7天内已更新或检测。" : "粘贴厂家发来的整段资料，确认一次即可保存。"}</p>{view === "products" && <button onClick={() => openQuickEntry()}>添加第一款产品</button>}</div>
              )}
            </div>
          </>
        )}
        ...
        <div className="quick-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) closeQuickEntry(); }}>
          <section className="quick-dialog" role="dialog" aria-modal="true" aria-labelledby="quick-entry-title">
            <header className="quick-dialog-header">
              <div><span>{editingProductId ? "编辑产品" : "快速录入"}</span><h2 id="quick-entry-title">{editingProductId ? `修改“${database.products.find((product) => product.id === editingProductId)?.name ?? "产品"}”` : "粘贴整段资料，一次确认"}</h2><p>{editingProductId ? "名称、单品价、推广状态、套餐、平台链接、机制、佣金和图片都可以一次修改。" : "自动拆分产品、单品价、推广状态、可选套餐、平台链接、机制和佣金。"}</p></div>
              ...
              <form className="quick-form" onSubmit={saveQuickEntry} onPaste={pasteQuickImage}>
                ...
                <section className="quick-section quick-basics-section">
                  ...
                  <div className="quick-package-block">
                    ...
                    {quickEntry.packages.length > 0 && <div className="quick-package-list">
                      {quickEntry.packages.map((item, index) => <div className="quick-package-row" key={item.id}>
                        <span>{String(index + 1).padStart(2, "0")}</span>
                        <label className={item.name.trim() ? "" : "missing"}>套餐名称 <em>必填</em><input value={item.name} onChange={(event) => updateQuickPackage(item.id, { name: event.target.value })} placeholder="例如：2盒装" /></label>
                        <label className={validQuickPrice(item.price) ? "" : "missing"}>套餐价格（元） <em>必填</em><input type="number" min="0.01" max="99999999" step="0.01" value={item.price} onChange={(event) => updateQuickPackage(item.id, { price: event.target.value })} placeholder="69.9" /></label>
                        <label>套餐内容 <small>选填</small><input value={item.description} onChange={(event) => updateQuickPackage(item.id, { description: event.target.value })} placeholder="例如：拍2发3" /></label>
                        <button type="button" aria-label={`移除套餐${index + 1}`} onClick={() => setQuickEntry((current) => ({ ...current, packages: current.packages.filter((currentItem) => currentItem.id !== item.id) }))}>×</button>
                      </div>)}
                      {quickHasDuplicatePackages && <small className="duplicate-tip">存在重复套餐名称，请合并或修改后再保存</small>}
                    </div>}
                  </div>
                ...
                <section className="quick-section quick-platforms-section">
                  <div className="quick-links-heading">
                    <div><span>02</span><strong>平台资料</strong><em>{quickEntry.links.length} 个平台</em></div>
                    <button type="button" onClick={addQuickPlatform}>＋ 添加平台</button>
                  </div>

                  {quickEntry.links.length > 1 && <details className="quick-batch-tools">
                    <summary>批量填写相同的机制和佣金</summary>
                    <section className="apply-all-panel">
                      <div><label>统一产品机制<textarea value={sharedMechanism} onChange={(event) => setSharedMechanism(event.target.value)} placeholder="规格、买赠活动等" /></label><button type="button" disabled={!sharedMechanism.trim()} onClick={() => setQuickEntry((current) => ({ ...current, links: current.links.map((link) => ({ ...link, mechanism: sharedMechanism.trim() })) }))}>应用全部</button></div>
                      <div><label>统一佣金（%）<input type="number" min="0" max="100" step="0.01" value={sharedCommission} onChange={(event) => setSharedCommission(event.target.value)} placeholder="例如 35" /></label><button type="button" disabled={!validQuickCommission(sharedCommission)} onClick={() => setQuickEntry((current) => ({ ...current, links: current.links.map((link) => ({ ...link, commission: sharedCommission })) }))}>应用全部</button></div>
                    </section>
                  </details>}

                  <div className="quick-link-list">
                    {quickEntry.links.map((link) => {
                      const complete = Boolean(link.url.trim() && link.mechanism.trim() && validQuickCommission(link.commission));
                      const active = activeQuickLinkId === link.id;
                      const duplicateLinkMatch = quickDuplicateCheck.urlMatches.find((match) => match.link.url.trim() === link.url.trim());
                      const duplicateWithinEntry = quickDuplicateCheck.duplicateUrls.includes(link.url.trim());
                      return <article className={`quick-link-card ${active ? "active" : ""} ${duplicateLinkMatch || duplicateWithinEntry ? "duplicate" : ""}`} key={link.id}>
                        <div className="quick-platform-head">
                          <button type="button" className="quick-platform-toggle" onClick={() => setActiveQuickLinkId(active ? null : link.id)}>
                            ...
                            <span className="quick-platform-summary"><strong>{link.url.trim() || "链接待填写"}</strong><small>{link.mechanism.trim() || "机制待填写"}</small></span>
                            <em className={complete ? "complete" : "incomplete"}>{complete ? `${link.commission}%` : "待补充"}</em>
                            <b>{active ? "收起" : "编辑"}</b>
                          </button>
                        ...
                        {active && <div className="quick-row-grid">
                          ...
                          <label className={link.url.trim() ? "" : "missing"}>链接 <em>必填</em><input value={link.url} onChange={(event) => updateQuickLink(link.id, { url: event.target.value })} placeholder="任意非空链接内容" />
                            {duplicateLinkMatch && <small className="duplicate-tip">该链接已用于“{duplicateLinkMatch.product.name}”的{duplicateLinkMatch.link.platform || "其他"}平台</small>}
                            {duplicateWithinEntry && <small className="duplicate-tip">这条链接在当前产品中重复填写</small>}
                          </label>
                          <label className={`quick-mechanism-field ${link.mechanism.trim() ? "" : "missing"}`}>产品机制 <em>必填</em><textarea value={link.mechanism} onChange={(event) => updateQuickLink(link.id, { mechanism: event.target.value })} placeholder="规格、买赠活动等" /></label>
                          <label className={validQuickCommission(link.commission) ? "" : "missing"}>佣金（%） <em>必填</em><input type="number" min="0" max="100" step="0.01" value={link.commission} onChange={(event) => updateQuickLink(link.id, { commission: event.target.value })} placeholder="35" />{validQuickCommission(link.commission) && Number(link.commission) < 30 && <small className="low-commission-tip">低于规定水平，仍可保存</small>}</label>
                        </div>}
                      </article>;
                    })}
                  </div>
          ...
          <section className="batch-dialog" role="dialog" aria-modal="true" aria-labelledby="batch-import-title">
            ...
            {!batchEntries.length ? <div className="batch-source-body">
              ...
              {batchSource === "text" ? <section className="batch-text-source">
                ...
                <small>必需列：产品名称、单品价格、链接、产品机制、佣金；套餐为选填，同一产品的多个平台请分多行填写。</small>
              </section>}
              ...
              <div className="batch-card-list">
                {batchEntries.map((entry, index) => {
                  ...
                  const lowCommission = entry.links.some((link) => validQuickCommission(link.commission) && Number(link.commission) < 30);
                    ...
                    {active && <div className="batch-product-editor">
                      ...
                      <div className="batch-link-editors">
                        {entry.links.map((link, linkIndex) => <section key={link.id}>
                          ...
                          <div className="batch-link-fields">
                            ...
                            <label className={link.url.trim() ? "" : "missing"}>链接 <em>必填</em><input value={link.url} onChange={(event) => updateBatchLink(entry.id, link.id, { url: event.target.value })} /></label>
                            <label className={link.mechanism.trim() ? "" : "missing"}>产品机制 <em>必填</em><textarea value={link.mechanism} onChange={(event) => updateBatchLink(entry.id, link.id, { mechanism: event.target.value })} /></label>
                            <label className={validQuickCommission(link.commission) ? "" : "missing"}>佣金（%） <em>必填</em><input type="number" min="0" max="100" step="0.01" value={link.commission} onChange={(event) => updateBatchLink(entry.id, link.id, { commission: event.target.value })} />{validQuickCommission(link.commission) && Number(link.commission) < 30 && <small className="low-commission-tip">低于规定水平，仍可导入</small>}</label>
                          </div>
                        </section>)}
                      </div>
                    </div>}
...
function ProductResult({ product, onCopy, onCheck, onEdit, checking }: { product: Product; onCopy: (value: string) => void; onCheck: (productId: string, linkId: string, url: string) => void; onEdit: (product: Product) => void; checking: string | null }) {
  const promotionStatus = product.status ?? "正常推广";
  return <article className="result-card">
    <header><div className="result-product-title">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : <div className="result-product-placeholder">{product.name.slice(0, 1)}</div>}<div><span className="product-kicker">产品</span><h3>{product.name}</h3><span className={productStatusClass(promotionStatus)}>{promotionStatus}</span></div></div><div className="result-product-meta"><strong>单品 {formatProductPrice(product.price)}</strong><small>{product.manufacturer}</small><button className="result-edit-button" onClick={() => onEdit(product)}>编辑资料</button></div></header>
    ...
    {product.links.map((link) => <div className="result-link" key={link.id}>
      ...
      {staleLinkDays(link) > 7 && <div className="stale-link-warning"><Icon name="history" /><span>该链接已 {staleLinkDays(link)} 天未更新，上品前请检查链接及佣金</span></div>}
      <button className="url-line" title={link.url} onClick={() => void onCopy(link.url)}><span>{link.url}</span><Icon name="copy" /></button>
      <div className="mechanism"><span>产品机制</span><p>{link.mechanism}</p></div>
      <div className={`commission-line ${link.commission < 30 ? "low" : ""}`}><span>佣金</span><strong>{Number.isFinite(link.commission) ? `${link.commission}%` : "待补充"}</strong>{link.commission < 30 && <em>低于规定水平</em>}</div>
      <div className="card-actions"><button onClick={() => void onCopy(link.url)}><Icon name="copy" />复制链接</button>{openableHref(link.url) && <a href={openableHref(link.url)!} target="_blank" rel="noreferrer"><Icon name="open" />打开链接</a>}<button disabled={checking === link.id} onClick={() => void onCheck(product.id, link.id, link.url)}>{checking === link.id ? "检测中…" : "检测"}</button></div>
    </div>)}
  </article>;
}

function ProductRow({ product, onCopy, onCheck, onEdit, onStatusChange, statusChanging, onDelete, onImageChange, onImageRemove, onEditMechanism, checking, canDelete }: { product: Product; onCopy: (value: string) => void; onCheck: (productId: string, linkId: string, url: string) => void; onEdit: (product: Product) => void; onStatusChange: (product: Product, status: ProductStatus) => void; statusChanging: boolean; onDelete: (product: Product) => void; onImageChange: (product: Product, file: File) => void; onImageRemove: (product: Product) => void; onEditMechanism: (platform: string) => void; checking: string | null; canDelete: boolean }) {
  const [open, setOpen] = useState(true);
  const [statusMenuOpen, setStatusMenuOpen] = useState(false);
  const imageInput = useRef<HTMLInputElement>(null);
  const promotionStatus = product.status ?? "正常推广";
  return <article className={`product-row ${statusMenuOpen ? "status-menu-open" : ""}`}>
    <header onClick={() => { setOpen(!open); setStatusMenuOpen(false); }}><div className="product-avatar">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : product.name.slice(0, 1)}</div><div className="product-title"><h3>{product.name}</h3><p>{product.manufacturer} · 单品 {formatProductPrice(product.price)}{product.packages.length ? ` · ${product.packages.length} 个套餐` : ""} · {product.links.length} 个平台</p></div><div className="platform-tags">{product.links.slice(0, 3).map((link) => <span key={link.id}>{link.platform}</span>)}</div><div className={`product-status-switch ${statusMenuOpen ? "open" : ""}`} onClick={(event) => event.stopPropagation()}><button type="button" className={productStatusClass(promotionStatus)} disabled={statusChanging} aria-haspopup="menu" aria-expanded={statusMenuOpen} onClick={() => setStatusMenuOpen((current) => !current)}>{statusChanging ? "保存中…" : promotionStatus}<span>⌄</span></button>{statusMenuOpen && <div role="menu" aria-label={`切换${product.name}的推广状态`}>{PRODUCT_STATUSES.map((status) => <button type="button" role="menuitem" className={promotionStatus === status ? "active" : ""} key={status} onClick={() => { setStatusMenuOpen(false); onStatusChange(product, status); }}><i className={`status-dot status-dot-${status}`} />{status}{promotionStatus === status && <Icon name="check" />}</button>)}</div>}</div><button className="product-edit-button" onClick={(event) => { event.stopPropagation(); onEdit(product); }}>编辑</button><button className={`chevron ${open ? "open" : ""}`} aria-label={open ? "收起产品资料" : "展开产品资料"}>⌄</button></header>
    ...
    {open && <div className="product-details">{productStatusWarning(promotionStatus) && <div className={`promotion-warning ${promotionStatus === "已下架" ? "danger" : "warning"}`}><Icon name="alert" /><strong>{productStatusWarning(promotionStatus)}</strong></div>}{product.packages.length > 0 && <div className="product-package-detail"><strong>可选套餐</strong><div>{product.packages.map((item) => <span key={item.id}><b>{item.name}</b><em>{formatProductPrice(item.price)}</em>{item.description && <small>{item.description}</small>}</span>)}</div></div>}{product.links.map((link) => <section key={link.id}>
      <div className="detail-top"><div><strong>{link.platform}</strong><span className={statusClass(link.status)}>{link.status}</span></div><small>更新于 {friendlyDate(link.updatedAt)}</small>{staleLinkDays(link) > 7 && <div className="stale-link-warning"><Icon name="history" /><span>该链接已 {staleLinkDays(link)} 天未更新，上品前请检查链接及佣金</span></div>}</div>
      <div className="detail-url"><span>{link.url}</span><button onClick={() => void onCopy(link.url)}><Icon name="copy" /></button>{openableHref(link.url) && <a href={openableHref(link.url)!} target="_blank" rel="noreferrer"><Icon name="open" /></a>}</div>
      <div className="detail-mechanism"><div><span>产品机制</span><button onClick={() => onEditMechanism(link.platform)}>修改</button></div><p>{link.mechanism}</p></div>
      <div className={`detail-commission ${link.commission < 30 ? "low" : ""}`}><span>佣金</span><strong>{Number.isFinite(link.commission) ? `${link.commission}%` : "待补充"}</strong>{link.commission < 30 && <small>低于规定水平</small>}</div>
      <div className="check-line"><span>{link.checkNote ?? "尚未执行网页可访问性检测"}</span><button disabled={checking === link.id} onClick={() => void onCheck(product.id, link.id, link.url)}>{checking === link.id ? "正在检测" : "检测链接"}</button></div>
    </section>)}<footer className="product-row-actions"><button className="product-image-button" onClick={() => imageInput.current?.click()}>{product.imageUrl ? "更换产品图片" : "添加产品图片"}</button>{product.imageUrl && <button className="product-image-remove" onClick={() => onImageRemove(product)}>移除图片</button>}{canDelete && <button className="delete-button" onClick={() => onDelete(product)}>删除产品</button>}</footer></div>}
  </article>;
}
